...
async function predict() {
  ...
  var celeb =
    "<div class='" +
    prediction[0].className +
    "-face-celeb face-celeb none'>" +
    resultCeleb +
    "</div>";
  const btn =
    "<span class='celeb-btn' style='text-decoration: underline;'>헤어 스타일 추천 보기 < </span>";
  $(".result-message").html(title + explain + btn + celeb);
  const celebBtn = document.querySelector(".celeb-btn");
  celebBtn.addEventListener("click", () => {
    if (!isCelebClick) {
      celebBtn.innerText = "헤어 스타일 추천 보기 > ";
      isCelebClick = true;
    } else {
      celebBtn.innerText = " 헤어 스타일 추천 보기 < ";
      isCelebClick = false;
    }
    document.querySelector(".face-celeb").classList.toggle(NONE_CN);
  });
  var barWidth;
  for (let i = 0; i < maxPredictions; i++) {
